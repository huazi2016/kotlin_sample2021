 ...
 * author : huazi
 * time   : 2020/9/3
 * desc   : 应用程序 Activity 管理类 (强烈建议在 Application$onCreate 方法中调用 register 方法注册，否则会导致 Activity 管理不完整)
 */
@Suppress("unused")
object AppManager {
    ...
    private val mActivityLifecycleCallbacks = object : Application.ActivityLifecycleCallbacks {
        ...
        override fun onActivityCreated(activity: Activity, savedInstanceState: Bundle?) {
            onCreate(activity)
            Logger.i("AppManager--onActivityCreated==" + activity.javaClass.name)
        }

        override fun onActivitySaveInstanceState(activity: Activity, outState: Bundle) {
            Logger.i("AppManager--onActivitySaveInstanceState==" + activity.javaClass.name)
        }
        ...
        override fun onActivityStarted(activity: Activity) {
            ...
            foregroundCount++
            Logger.i("AppManager--onActivityStarted==" + activity.javaClass.name)
        }

        override fun onActivityResumed(activity: Activity) {
            Logger.i("AppManager--onActivityResumed==" + activity.javaClass.name)
        }

        override fun onActivityPaused(activity: Activity) {
            Logger.i("AppManager--onActivityPaused==" + activity.javaClass.name)
        }
        ...
        override fun onActivityStopped(activity: Activity) {
            ...
            Logger.i("AppManager--onActivityStopped==" + activity.javaClass.name)
        }

        override fun onActivityDestroyed(activity: Activity) {
            onDestroy(activity)
            Logger.i("AppManager--onActivityDestroyed==" + activity.javaClass.name)
        }
    }
    ...
    private fun getApplicationByReflect(): Application {
        try {
            ...
            Logger.e("AppManager", "getApplicationByReflect", e)
        }
        throw NullPointerException("u should init first")
    }
