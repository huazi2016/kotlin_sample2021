 ...
 * desc   : 高斯Transform
 */
class BlurTransform @JvmOverloads constructor(private val context: Context, radius: Int = MAX_RADIUS, sampling: Int = DEFAULT_SAMPLING)
    ...
    override fun transform(pool: BitmapPool, toTransform: Bitmap, outWidth: Int, outHeight: Int): Bitmap {
        ...
        bitmap = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR1) {
            BlurTool.rsBlur(context, bitmap, radius)
        } else {
            BlurTool.blur(bitmap, radius)!!
        }
        return bitmap
    }

    override fun equals(obj: Any?): Boolean {
        if (obj is BlurTransform) {
            val other = obj
